...
# dann Server starten.
CMD ["sh", "-c", "node node_modules/prisma/build/index.js migrate deploy && { [ \"$SEED_DEMO\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs prisma/seed-demo.ts || true; } && { [ -n \"$SELFTEST\" ] && node node_modules/tsx/dist/cli.mjs scripts/selftest.ts || true; } && { [ \"$VALIDATE\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs scripts/validate-sevdesk.ts || true; } && { [ \"$DUMP_CP\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs scripts/dump-counterparties.ts || true; } && { [ \"$SEED_RULES\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs scripts/seed-rules.ts || true; } && { [ \"$LOADTEST\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs scripts/loadtest.ts || true; } && { [ \"$DIGEST_TEST\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs scripts/digest-test.ts || true; } && { [ \"$DB_MAINT\" = \"true\" ] && node node_modules/tsx/dist/cli.mjs scripts/db-maint.ts || true; } && node server.js"]
